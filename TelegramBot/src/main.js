...
bot.hears('TF-IDF', async (ctx) => {
  try {
    ...
    const csvData = await readCSV('./bd_bot/tfidf.csv')
...
bot.hears('BOWords', async (ctx) => {
  try {
    ...
    const csvData = await readCSV('./bd_bot/bowords.csv')
    const formattedData = formatCSVData(csvData)
...
bot.hears('Word2Vec', async (ctx) => {
  try {
    ...
    const csvData = await readCSV('./bd_bot/word2vec.csv')
    const formattedData = formatCSVData(csvData)
...
bot.hears('DistilBERT', async (ctx) => {
  try {
    ...
    const csvData = await readCSV('./bd_bot/distilbert.csv')
    const formattedData = formatCSVData(csvData)
