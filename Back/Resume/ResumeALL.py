...
#         all_resume_data = all_resume_data.applymap(lambda x: re.sub(r'\s+', ' ', x.strip()) if isinstance(x, str) else x)
...
#         all_resume_data['INF'] = all_resume_data.apply(lambda row: f"{row['achievements']} {row['city']}  {row['experience']} \
...
#             try:
#                 all_resume_data['CATEGORY'] = int(input())
#                 break
#             except:
#                 print('Error : некорректный ввод\n.')
#                 continue
#         all_resume_data = all_resume_data[['CATEGORY','name', 'INF', 'link', 'ID']]
#         all_resume_data.to_csv('./Back/bd/all_resume.csv', mode='a', index=False, header=not os.path.exists('./Back/bd/all_resume_new.csv'))
...
class Resume:
    ...
    def get_resume_links(self, page_range=(1, 1)):
        def process_page(page_num):
            page_url = f"https://moskva.gorodrabot.ru/resumes?q={self.params}&p={page_num}"
            response = requests.get(page_url)
            soup = BeautifulSoup(response.content, 'html.parser')
            page_links = set()
            for link in soup.find_all('a'):
    ...
    def get_links_for_params(self):
        return self.get_resume_links(page_range=(1, self.page_range_max))
    ...
    def main(self):
        ...
        all_resume_data = all_resume_data[all_resume_data['name'] != '']
        all_resume_data['ID'] = all_resume_data.get('link').apply(lambda row: re.search(r'\d+', row).group())
        all_resume_data['INF'] = all_resume_data.apply(lambda row: f"{row['achievements']} {row['city']}  \
{row['experience']} {row['education']} {row['schedule']} {row['about']} {row['skills']} {row['info']}", axis=1)
        while True:
            print('Введите номер категории для данного типа резюме из сайта hh.ru:')
            try:
                all_resume_data['CATEGORY'] = int(input())
                break
            except:
                ...
                continue
        all_resume_data = all_resume_data[['CATEGORY', 'name', 'INF', 'link', 'ID']]
        all_resume_data.to_csv('./Back/bd/all_resume.csv', mode='a', index=False, header=False)
        return all_resume_data
#
#
# resume = Resume(params="аудитор", page_range_max=4, max_workers=4)
# resume.main()
